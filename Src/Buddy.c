...
**          
** $Source: /mit/games/src/vax/xtank1.2/Src/RCS/Buddy.c,v $
** $Author: dschmidt $
** $Revision: 1.4 $
**
** $Log:	Buddy.c,v $
     ...
     Binfo *bi;
{
  ...
  if (msgs) {
	...
	send_msg(bi->msg.sender, OP_LOCATION, data);
      }
    }
  }
  if (!bi->numdiscs)
    /* Ask for help */
    send_msg(MAX_VEHICLES+bi->me.team,OP_HELP,data);
}
     ...
     Goal_info *gi;
{
  ...
  if (!Buddy_wall(bi,NORTH,gi->loc.grid_x,gi->loc.grid_y)) {
  ...
  if (!Buddy_wall(bi,SOUTH,gi->loc.grid_x,gi->loc.grid_y)) {
    g1 = &bi->goal_locs_1[bi->num_goal_locs];
    g2 = &bi->goal_locs_2[bi->num_goal_locs++];
    g1->grid_x = gi->loc.grid_x;
    g1->grid_y = gi->loc.grid_y;
  ...
  if (!Buddy_wall(bi,WEST,gi->loc.grid_x,gi->loc.grid_y)) {
    g1 = &bi->goal_locs_1[bi->num_goal_locs];
    g2 = &bi->goal_locs_2[bi->num_goal_locs++];
    g1->grid_x = gi->loc.grid_x;
    g1->grid_y = gi->loc.grid_y;
  ...
  if (!Buddy_wall(bi,EAST,gi->loc.grid_x,gi->loc.grid_y)) {
    g1 = &bi->goal_locs_1[bi->num_goal_locs];
    g2 = &bi->goal_locs_2[bi->num_goal_locs++];
    g1->grid_x = gi->loc.grid_x;
    g1->grid_y = gi->loc.grid_y;
...
#define set_locs(x1,y1,x2,y2) \
  ...
  if (gy2 == gy)
    ...
    if (gy2 == my + 1)		/* southeast */
      if (Buddy_wall(bi,NORTH,gx2,gy2))
	set_locs(gx2, gy2, gx2, gy2 + 1)
      else if (Buddy_wall(bi,WEST,gx2,gy2))
	set_locs(gx2, gy2, gx2 + 1, gy2)
      else
	set_locs(gx2, gy2 + 1, gx2 + 1, gy2)
    else			/* northeast */
      if (Buddy_wall(bi,SOUTH,gx2,gy2))
	...
	set_locs(gx2 + 1, gy2, gx2 + 1, gy2 + 1)
      else if (Buddy_wall(bi,EAST,gx2,gy2))
	set_locs(gx2, gy2, gx2 + 1, gy2)
      else
	set_locs(gx2, gy2, gx2 + 1, gy2 + 1)
    else			/* northwest */
     ...
     float speed;
{
  ...
  if (diff < THROW_THRESH || diff > 2*PI - THROW_THRESH) {
    throw_discs(speed,FALSE); bi->throwing = FALSE; return;
  }
  ...
  if (diff2 < THROW_THRESH || diff2 > 2*PI - THROW_THRESH) {
    throw_discs(speed,TRUE); bi->throwing = FALSE; return;
  }
...
/* Buddy_throw_in_range(bi,loc1,loc2,tricky,speed)
**
     ...
     Binfo *bi;
{
  ...
  if (owner == NO_OWNER || team(owner) != bi->me.team) {
	...
	if (Buddy_wall(bi,EAST,oldx,oldy)) {
	...
	if (Buddy_wall(bi,WEST,oldx,oldy)) {
	...
	if (Buddy_wall(bi,SOUTH,oldx,oldy)) {
	...
	if (Buddy_wall(bi,NORTH,oldx,oldy)) {
  ...
  for (depth = 0; depth < maxdepth && num_on_edge > 0; ++depth) {
	...
	return(FALSE);
	 ...
	 square yet, flood into it */
      if (!Buddy_wall(bi,NORTH,cx,cy) && !maze[cx][cy-1].seen) {
	...
	next_cut[next_num_on_edge++].y = cy - 1;
      ...
      if (!Buddy_wall(bi,SOUTH,cx,cy) && !maze[cx][cy+1].seen) {
	maze[cx][cy+1].nextx = cx;
	maze[cx][cy+1].nexty = cy;
	maze[cx][cy+1].seen = TRUE;
	maze[cx][cy+1].dist = depth + 1;
	...
	next_cut[next_num_on_edge++].y = cy + 1;
      ...
      if (!Buddy_wall(bi,WEST,cx,cy) && !maze[cx-1][cy].seen) {
	...
	next_cut[next_num_on_edge++].y = cy;
      ...
      if (!Buddy_wall(bi,EAST,cx,cy) && !maze[cx+1][cy].seen) {
	maze[cx+1][cy].nextx = cx;
	maze[cx+1][cy].nexty = cy;
	maze[cx+1][cy].seen = TRUE;
	maze[cx+1][cy].dist = depth + 1;
     ...
     Binfo *bi;
{
  ...
  if (dir > PI/4 && dir < 3*PI/4) {                  /* south */
    if (Buddy_wall(bi,SOUTH,mx,my)) {
      if (Buddy_wall(bi,WEST,mx,my)) turn_vehicle(0.0);
      else if (Buddy_wall(bi,EAST,mx,my)) turn_vehicle(PI);
      else if (dir < PI/2) turn_vehicle(0.0);
      else turn_vehicle(PI);
    }
  }
  else if (dir >= 3*PI/4 && dir < 5*PI/4) {          /* west */
    if (Buddy_wall(bi,WEST,mx,my)) {
      if (Buddy_wall(bi,NORTH,mx,my)) turn_vehicle(PI/2);
      else if (Buddy_wall(bi,SOUTH,mx,my)) turn_vehicle(3*PI/2);
      else if (dir < PI) turn_vehicle(PI/2);
      else turn_vehicle(3*PI/2);
    }
  }
  else if (dir >= 5*PI/4 && dir < 7*PI/4) {          /* north */
    if (Buddy_wall(bi,NORTH,mx,my)) {
      if (Buddy_wall(bi,EAST,mx,my)) turn_vehicle(PI);
      else if (Buddy_wall(bi,WEST,mx,my)) turn_vehicle(0.0);
      else if (dir < 3*PI/2) turn_vehicle(PI/2);
      else turn_vehicle(0.0);
    }
  }
  else if (dir >= 3*PI/4 && dir < 5*PI/4) {          /* east */
    if (Buddy_wall(bi,EAST,mx,my)) {
      if (Buddy_wall(bi,SOUTH,mx,my)) turn_vehicle(3*PI/2);
      else if (Buddy_wall(bi,NORTH,mx,my)) turn_vehicle(PI/2);

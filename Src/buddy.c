...
int newthrow;
{
    ...
    alpha = ATAN2(loc.grid_y * BOX_HEIGHT + loc.box_y - bi->absloc.y,
...
int x, y;
{
	...
	{
		if (wall(NORTH, hx, hy))
			bi->maze[hx][hy] |= BUDDY_NORTH_WALL;
		else
			bi->maze[hx][hy] &= ~BUDDY_NORTH_WALL;
		if (wall(WEST, hx, hy))
			bi->maze[hx][hy] |= BUDDY_WEST_WALL;
		else
			bi->maze[hx][hy] &= ~BUDDY_WEST_WALL;
		bi->maze[hx][hy] |= SEEN;
	}
	if (hside == NORTH)
	{
		if (bi->maze[hx][hy] & BUDDY_NORTH_WALL)
			return (TRUE);
		else
			return (FALSE);
	}
	...
	{							/* hside == WEST */
		if (bi->maze[hx][hy] & BUDDY_WEST_WALL)
			return (TRUE);
		else
			return (FALSE);
	}
...
Angle dir;
{
	...
	switch (corner)
	{
		case NW:
			if (buddy_wall(bi, NORTH, gx, gy))
			{
				if (buddy_wall(bi, WEST, gx, gy))
                    doturnstuff(ATAN2(BOX_HEIGHT - by, bx), PI, -1,
                                ATAN2(BOX_WIDTH - bx, by), 3 * PI / 2, 1);
				else
				{
					if (buddy_wall(bi, NORTH, gx - 1, gy))
                        doturnstuff(ATAN2(BOX_WIDTH + bx, by), 3 * PI / 2, -1,
                                ATAN2(BOX_WIDTH - bx, by), 3 * PI / 2, 1);
					else
                        doturnstuff(ATAN2(bx, by), 3 * PI / 2, -1,
                                ATAN2(BOX_WIDTH - bx, by), 3 * PI / 2, 1);
				}
			}
			else if (buddy_wall(bi, WEST, gx, gy))
			{
				if (buddy_wall(bi, WEST, gx, gy - 1))
                    doturnstuff(ATAN2(BOX_HEIGHT - by, bx), PI, -1,
                                ATAN2(BOX_HEIGHT + by, bx), PI, 1);
				else
                    doturnstuff(ATAN2(BOX_HEIGHT - by, bx), PI, -1,
                                ATAN2(bx, by), 3 * PI / 2, -1);
			}
		...
		case SW:
			...
			{
				if (buddy_wall(bi, WEST, gx, gy))
                    doturnstuff(ATAN2(BOX_WIDTH - bx, BOX_HEIGHT - by), PI / 2, -1,
                                ATAN2(by, bx), PI, 1);
				else
				{
					if (buddy_wall(bi, SOUTH, gx - 1, gy))
                        doturnstuff(ATAN2(BOX_WIDTH - bx, BOX_HEIGHT - by), PI / 2, -1,
                        ATAN2(BOX_WIDTH + bx, BOX_HEIGHT - by), PI / 2, 1);
					else
                        doturnstuff(ATAN2(BOX_WIDTH - bx, BOX_HEIGHT - by), PI / 2, -1,
                                    ATAN2(bx, BOX_HEIGHT - by), PI / 2, 1);
				}
			}
			else if (buddy_wall(bi, WEST, gx, gy))
			{
				if (buddy_wall(bi, WEST, gx, gy + 1))
                    doturnstuff(ATAN2(BOX_HEIGHT * 2 - by, bx), PI, -1,
                                ATAN2(by, bx), PI, 1);
				else
                    doturnstuff(ATAN2(bx, BOX_HEIGHT - by), PI / 2, 1,
                                ATAN2(by, bx), PI, 1);
			}
			else if (buddy_wall(bi, WEST, gx, gy + 1))
			{
		...
		case NE:
			if (buddy_wall(bi, NORTH, gx, gy))
			{
				if (buddy_wall(bi, EAST, gx, gy))
                    doturnstuff(ATAN2(bx, by), 3 * PI / 2, -1,
                       ATAN2(BOX_HEIGHT - by, BOX_WIDTH - bx), 2 * PI, 1);
				else
				{
					if (buddy_wall(bi, NORTH, gx + 1, gy))
                        doturnstuff(ATAN2(bx, by), 3 * PI / 2, -1,
                            ATAN2(2 * BOX_WIDTH - bx, by), 3 * PI / 2, 1);
					else
                        doturnstuff(ATAN2(bx, by), 3 * PI / 2, -1,
                                ATAN2(BOX_WIDTH - bx, by), 3 * PI / 2, 1);
				}
			...
			else if (buddy_wall(bi, EAST, gx, gy))
			{
				if (buddy_wall(bi, EAST, gx, gy - 1))
                    doturnstuff(ATAN2(BOX_HEIGHT + by, BOX_WIDTH - bx), 2 * PI, -1,
                       ATAN2(BOX_HEIGHT - by, BOX_WIDTH - bx), 2 * PI, 1);
				else
                    doturnstuff(ATAN2(BOX_WIDTH - bx, by), 3 * PI / 2, 1,
                       ATAN2(BOX_HEIGHT - by, BOX_WIDTH - bx), 2 * PI, 1);
		...
		case SE:
			...
			{
				if (buddy_wall(bi, EAST, gx, gy))
                    doturnstuff(ATAN2(by, BOX_WIDTH - bx), 2 * PI, -1,
                                ATAN2(bx, BOX_HEIGHT - by), 5 * PI / 2, 1);
				else
				{
					if (buddy_wall(bi, SOUTH, gx + 1, gy))
                        doturnstuff(ATAN2(2 * BOX_WIDTH - bx, BOX_HEIGHT - by), PI / 2, -1,
                                    ATAN2(bx, BOX_HEIGHT - by), PI / 2, 1);
					else
                        doturnstuff(ATAN2(BOX_HEIGHT - by, BOX_WIDTH - bx), 0, 1,
                                    ATAN2(bx, BOX_HEIGHT - by), PI / 2, 1);
				}
			}
			else if (buddy_wall(bi, EAST, gx, gy))
			{
				if (buddy_wall(bi, EAST, gx, gy + 1))
                    doturnstuff(ATAN2(by, BOX_WIDTH - bx), 2 * PI, -1,
                    ATAN2(2 * BOX_HEIGHT - by, BOX_WIDTH - bx), 2 * PI, 1);
				else
                    doturnstuff(ATAN2(by, BOX_WIDTH - bx), 2 * PI, -1,
                       ATAN2(BOX_HEIGHT - by, BOX_WIDTH - bx), 2 * PI, 1);
			}
			...
			{
				if (buddy_wall(bi, SOUTH, gx + 1, gy))
                    doturnstuff(ATAN2(2 * BOX_WIDTH - bx, BOX_HEIGHT - by),
				PI / 2, -1,
                        ATAN2(2 * BOX_HEIGHT - by, BOX_WIDTH - bx), 0, 1);
				else
                    doturnstuff(ATAN2(BOX_HEIGHT - by, BOX_WIDTH - bx), 0, 1,
                        ATAN2(2 * BOX_HEIGHT - by, BOX_WIDTH - bx), 0, 1);
			}
                ...
                doturnstuff(ATAN2(2 * BOX_WIDTH - bx, BOX_HEIGHT - by),
			    PI / 2, -1,
                            ATAN2(BOX_HEIGHT - by, BOX_WIDTH - bx), 0, 1);
			break;
	...
	else
	{
		...
		if ((min_ang > 2 * PI || max_ang > 2 * PI) && idir < min_ang)
			idir += 2 * PI;

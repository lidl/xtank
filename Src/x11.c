...
static char *color_titles[MAX_COLORS] = {
	...
	"color.cursor",				/* Cursor, tink, commentator */
};
...
static char *colorname[MAX_COLORS] = {
	"Black", "White", "Red", "DarkOrange", "Yellow", "Green", "DodgerBlue",
	"Violet", "grey71", "cyan",
};
...
open_graphics()
{
	...
	printf("Setting up new error handler...\n");
...
char *name;
{
	...
	/********************************/
	/* XSynchronize(video->dpy, 1);		40 times slower, but debugable */
	/********************************/
...
/*
 * * Use this routine to draw text in an arbitrary position in a window * The
 * x coordinate specifies the center of the string to be drawn. * The y
...
/*
 * * Gets up to *num_events input events, handling the expose events * by
 * setting the WIN_expose flag.  Events are stored in the event * array, with
 * num_events holding the number of events placed there.
 */
...
make_gcs()
{
	...
	if (vid->planes > 1)
	{
		...
		{
			cptr = get_default(color_titles[i], color_classes[i], colorname[i]);
...
unsigned char *bitmap;
{
	...
	/* 319 */
	if (!XQueryBestSize(vid->dpy, TileShape, RootWindow(vid->dpy, 
		DefaultScreen(vid->dpy)), pic->width, pic->height, &w, &h)) {
	...
	if (w < pic->width || h < pic->height)
	{
		...
		fprintf(stderr, "It gave new size smaller than the original.\n\n");
		w = pic->width;
		h = pic->height;
	}

	pix = XCreatePixmap(vid->dpy, RootWindow(vid->dpy, DefaultScreen(vid->dpy)),
		w, h, 1);
	if (!pix) {

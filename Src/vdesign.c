...
static char
*engine_entries[MAX_ENGINES], *weapon_entries[VMAX_WEAPONS + 1], *armor_entries[MAX_ARMORS], *body_entries[MAX_BODIES],
*special_entries[MAX_SPECIALS], *suspension_entries[MAX_SUSPENSIONS], *tread_entries[MAX_TREADS],
...
init_vdesign_interface()
{
	...
	menu_norm_make(&menu_sys, SUSPENSIONS_MENU, "Suspensions", MAX_SUSPENSIONS, 0,
			...
			case EVENT_RBUTTON:
				...
				switch (menu)
				{
					...
					case WEAPNUMS_MENU:
						...
						if (weapnum < d->num_weapons)
                            menu_set_hil(&menu_sys, WEAPONS_MENU,
					 d->weapon[weapnum]);
...
Vdesc *d;
{
	...
	d->tread_acc = friction * MAX_ACCEL;
...
Vdesc *d;
{
	...
	input_string(ANIM_WIN, "Enter vehicle name:", name, 0, INPUT_ROW, VDESIGN_FONT, 256);
	if (name[0] != '\0')
	{

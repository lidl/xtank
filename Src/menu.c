 ...
 */
menu_bare_make(menuobj, menuid, title, size, width,
			   xtop, ytop, fntcode, has_bar, use_escher)
			...
			if (menu_hit_in_border(menuobj, menuid, x, y))
			{
				...
				{				/* THE PICK */
					*selec = menu_track_mouse(menuobj, menuid, y);
					if (*selec == -1)
					{			/* track_mouse returns it ~ left screen */
						*p_menuid = MENU_NULL;
						return;
					}
				}
		...
		if (numev == 1)
		{
			switch (ev.type)
			{
				...
				case EVENT_MOVED:
					...
					if (menu_hit_in_border(menuobj, menuid, x, y))
					{
						if ((newchoice = menu_resolve_coord(menuobj, menuid, y)) == curchoice)
							break;
						else
						{
...
menu_highlight(w, m, item)		/* Takes item as nth menu item.. not nth
								   field */
int w;
Menu *m;
int item;
{
	...
	/* m->xt + 1, m->yt + (m->fh * (item - m->titled)), */
					 m->width - 1, m->fh,
					 DRAW_XOR, MENU_FORE);
}

     ...
     Location *start, *finish;
{
  ...
  /*
  ** This does not do the "obvious" thing.  It does the fast thing.
  ** It compares the slopes of lines (from the destination to gridpoints
  ** along the path) to the line of the path.  From these comparisons,
  ** it determines which gridpoint to walk to next.  It does NOT walk
  ** from box to box, but from gridpoint to gridpoint.
  */
  ...
  */
  if(fgrid_x > tgrid_x)
	...
	if(lattice_dy * dx < dy * lattice_dx) {
	  if(wall(SOUTH,tgrid_x,tgrid_y)) return FALSE;
	  tgrid_y++;
	  lattice_dy += BOX_HEIGHT;
	}
	else {
	  if(wall(EAST,tgrid_x,tgrid_y)) return FALSE;
	  tgrid_x++;
      ...
      while(tgrid_x != fgrid_x || tgrid_y != fgrid_y) {
	if(lattice_dy * dx > dy * lattice_dx) {
	  if(wall(NORTH,tgrid_x,tgrid_y)) return FALSE;
	  tgrid_y--;
	  lattice_dy -= BOX_HEIGHT;
	}
	else {
	...
	if(lattice_dy * dx >= dy * lattice_dx) {
	  ...
	  if(wall(WEST,tgrid_x,tgrid_y)) return FALSE;
	  tgrid_x--;
	  lattice_dx -= BOX_WIDTH;
	}
      }
     ...
     Vehicle_info *enemy;
{
  ...
  for(i = 0 ; i < num_vinfos ; i++) {
    ...
    if(v->team == me.team && me.team != 0) continue;

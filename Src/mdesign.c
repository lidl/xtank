...
design_maze()
{
  ...
  do {                   
    ...
    switch ((int) event.type) {
      case EVENT_KEY:
        if(handle_key(&event) == GAME_QUIT) done = TRUE;
	break;
      case EVENT_LBUTTON:
      case EVENT_MBUTTON:
      case EVENT_RBUTTON:
        if(handle_button(&event) == GAME_QUIT) done = TRUE;
	break;
      }
  } while(done == FALSE);
     ...
     Event *event;
{
  ...
  switch(event->type) {
    case EVENT_LBUTTON:	change_maze(&loc,MAKE_NORM_WALL); break;
    case EVENT_MBUTTON:	change_maze(&loc,MAKE_DEST_WALL); break;
    case EVENT_RBUTTON: change_maze(&loc,DESTROY_WALL); break;
    }
     ...
     Event *event;
{
  ...
  switch (event->key) {
    case ' ': make_landmark(&bloc,NORMAL);    break;
    case 'a': make_landmark(&bloc,ARMOR);     break;
    case 'f': make_landmark(&bloc,FUEL);      break;
    case 'w': make_landmark(&bloc,AMMO);      break;
    case 'o': make_landmark(&bloc,OUTPOST);   break;
    case 'g': make_landmark(&bloc,GOAL);      break;
    case '-': make_landmark(&bloc,SLIP);      break;
    case '=': make_landmark(&bloc,SLOW);      break;
    case 'x': make_landmark(&bloc,START_POS); break;
    case '1': make_landmark(&bloc,SCROLL_SW); break;
    case '2': make_landmark(&bloc,SCROLL_S);  break;
    case '3': make_landmark(&bloc,SCROLL_SE); break;
    case '4': make_landmark(&bloc,SCROLL_W);  break;
    case '6': make_landmark(&bloc,SCROLL_E);  break;
    case '7': make_landmark(&bloc,SCROLL_NW); break;
    case '8': make_landmark(&bloc,SCROLL_N);  break;
    case '9': make_landmark(&bloc,SCROLL_NE); break;

    case ')': set_team(&bloc,0); break;
    case '!': set_team(&bloc,1); break;
    case '@': set_team(&bloc,2); break;
    case '#': set_team(&bloc,3); break;
    case '$': set_team(&bloc,4); break;
    case '%': set_team(&bloc,5); break;
    case '^': set_team(&bloc,6); break;

    case 'm': move_area(); break;
    case 'c': copy_area(); break;
    case 'e': erase_area(); break;
     ...
     Mdesc *d;
{
  ...
  sprintf(temp,"Name: %-15s  Type: %-10s  Designer: %-24s",
	  d->name,game_str[d->type],d->designer);
     ...
     int action;		/* was short */
{
  ...
  if ((i/DES_BOX_WIDTH<PAD) || (i/DES_BOX_WIDTH>GRID_WIDTH-PAD)||
      (j/DES_BOX_HEIGHT<PAD) || (j/DES_BOX_HEIGHT>GRID_HEIGHT-PAD)) return;

  /* See if location is within THRESHOLD pixels of a vertical wall */
  if ((i+THRESHOLD) % DES_BOX_WIDTH <= 2*THRESHOLD) {
    wall |= WEST_WALL;
    /* When we do an int division by DES_BOX_WIDTH, we'll get the right box */
    i += THRESHOLD;
  }
  ...
  */
  ...
  if(check_wall(&bloc,wall) == FALSE) return;
  ...
  */
  if ((action == CONTINUE) && (wall==cur_wall) &&
      (bloc.x == curbloc.x) && (bloc.y == curbloc.y)) return;

  /* We are on a new wall, so remember it for later.
  ** If the calling function sent a CONTINUE, a button is being held down.
  ** Otherwise, a button was just pressed.
  */
  if(action != CONTINUE) cur_action = action; 
  cur_wall = wall;  
  curbloc.x = bloc.x; 
  curbloc.y = bloc.y;
  ...
  switch(cur_action) {
    case MAKE_NORM_WALL: make_wall(&bloc,wall); break;
    case MAKE_DEST_WALL: make_destructible(&bloc,wall); break;
    case DESTROY_WALL: unmake_wall(&bloc,wall); break;
    }  
...
mdesign_save()
{
  ...
  if(figure_insideness() == FALSE)
    ...
    if(name[0] != '\0') {
      mprint("1) Combat  2) War  3) Ultimate  4) Capture  5) Race",
	    0,INPUT_Y+1);
      type = input_int(ANIM_WIN,"Enter maze type",0,INPUT_Y+2,
		       1,1,5,MD_FONT) - 1;
      input_string(ANIM_WIN,"Enter maze description:",desc,
		   0,INPUT_Y+3,MD_FONT);

      build_mdesc(&design_mdesc,(Byte) type,name,term->player_name,desc);
      display_mdesc_info(&design_mdesc);
      mdesign_clear_input();
  ...
  mprint("Any key or button to return",0,INPUT_Y+2);
  wait_input();
  mdesign_clear_input();
}
...
mdesign_load()
{
  ...
  input_string(ANIM_WIN,"Enter maze name:",name,0,INPUT_Y,MD_FONT);
  if(name[0] != '\0') {
     ...
     BoxC *loc;
{
  ...
  if (b->type != NORMAL) show_landmark(loc);
  if (b->flags & NORTH_WALL) show_wall(loc,NORTH_WALL);
  if (b->flags & WEST_WALL)  show_wall(loc,WEST_WALL);
}
     ...
     BoxC *loc;
{
  ...
  draw_picture(ANIM_WIN,ploc.x,ploc.y,pic,DRAW_COPY,WHITE);
  if (type != NORMAL) show_landmark(loc);
     ...
     BoxC bvertices[2];
{
  ...
  /*
  ** Make a rectangle to indicate what area he is selecting.
  ** Its corners are also at grid intersections.
  */
  ...
  do {
    ...
    if(event.type == EVENT_LBUTTONUP ||
       ...
       event.type == EVENT_RBUTTONUP) break;
  } while(TRUE);
...
figure_insideness()
{
  ...
  while(size) {
    ...
    if (!(flags & WEST_WALL) || flags & WEST_DEST) {
      ...
      else add_to_maze(i-1,j,boxes,&size);
    }

    if (!(flags & NORTH_WALL) || flags & NORTH_DEST) {
      if (j <= PAD) return FALSE;
      else add_to_maze(i,j-1,boxes,&size);
    }
    
    flags = box[i+1][j].flags;
    if (!(flags & WEST_WALL) || flags & WEST_DEST) {
      if (i >= GRID_WIDTH-PAD) return FALSE;
      else add_to_maze(i+1,j,boxes,&size);
    }
      
    flags = box[i][j+1].flags;
    if (!(flags & NORTH_WALL) || flags & NORTH_DEST) {
      if (j >= GRID_HEIGHT-PAD) return FALSE;
      else add_to_maze(i,j+1,boxes,&size);
    }
  }
     ...
     PixC *loc;
{
  if((loc->x < X_OFFSET + PAD*DES_BOX_WIDTH) ||
     ...
     (loc->y > Y_OFFSET + (GRID_HEIGHT-PAD)*DES_BOX_HEIGHT)) return FALSE;
  else return TRUE;
}
     ...
     BoxC *loc;
{
  if (loc->x < PAD || loc->x >= GRID_WIDTH-PAD ||
      loc->y < PAD || loc->y >= GRID_HEIGHT-PAD) return FALSE;
  else return TRUE;
}
     ...
     Wall wall;
{
  ...
  */
  if (box[iloc.x][iloc.y].team) show_box(&iloc);

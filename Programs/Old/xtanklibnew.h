...
#define map_wall(w,map,dir,x,y) if(map_off(x,y)) w = MAP_NONE;  \
  else switch(dir) { \
    case NORTH: w = map_north_result((map)[x][y].flags); break; \
    case SOUTH: w = map_north_result((map)[x][(y)+1].flags); break; \
    case EAST:  w = map_west_result((map)[(x)+1][y].flags); break; \
    case WEST:  w = map_west_result((map)[x][y].flags); break; \
  }

#ifdef notdef
#define map_wall(map,dir,x,y) \
  (map_off(x,y) ? MAP_NONE : \
...
#define map_landmark(map,x,y) (map_off(x,y) ? 0 : ((map)[x][y].type))
#define map_team(map,x,y)     (map_off(x,y) ? 0 : ((map)[x][y].team))
#define map_strength(map,x,y) (map_off(x,y) ? 0 : ((map)[x][y].strength))
#define map_inside(map,x,y) \
  (map_off(x,y) ? 0 : (map)[x][y].flags & INSIDE_MAZE)

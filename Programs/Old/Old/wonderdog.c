/*
** Yes, its the amazing WONDERDOG the tank!
** Randy Sargent
** 4/25/88
*/
...
void wonderdog_get_unstuck();
void wonderdog_move();
void wonderdog_shoot();
void wonderdog_wakeup();
int wonderdog_me[100]; 
...
wonderdog_main()
{
    ...
    while(1) {
#ifdef wonderdog_msg
      sprintf(temp,"rev is %d, ang %d",wonderdog_rev,bul_spd);
      message(temp);  
#endif
	
      if (speed() == 0) {
	  wonderdog_get_unstuck(bul_spd); wonderdog_rev= !wonderdog_rev;
      }
      wonderdog_move(wonderdog_rev);
  
      wonderdog_shoot(bul_spd);
    }
}

void wonderdog_move(rev)
int rev;
{
...
void wonderdog_get_unstuck(bul_spd)
int bul_spd;
{
...
void wonderdog_wakeup()
{
	...
	for (i=0; i<num; i++) {
		...
		wonderdog_me[vehicles[i].id]++;
	    } else {
		wonderdog_me[vehicles[i].id]= 0;
	    }
	}
	wonderdog_shoot(bul_spd);
    }
    for (i=0; i<100; i++) if (wonderdog_me[i]== 1) wonderdog_me[i]= 0;
}    
    
void wonderdog_shoot(bul_spd)
int bul_spd;
{
	...
	range= wonderdog_range(&vehicles[i].loc, &my_loc);
	speed= wonderdog_speed(&vehicles[i]);
	/* wouldn't it be embarrasing to shoot yourself? */
	if ((range < bestrange) && (kills || (!wonderdog_me[vehicles[i].id]))) {
	    v= &vehicles[i];
	    bestrange= range;
	}
    }

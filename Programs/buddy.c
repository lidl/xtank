...
#include "frog.h"		/* The constants, structs, etc. */
...
frog_main()
{
  ...
  frog_initialize_maze(&bi);	/* get the maze set up */
  frog_init_bi(&bi);		/* initialize the bi structure */
  ...
  while (TRUE) {
    ...
    frog_look_for_goals(&bi);

    switch(bi.mode) {
    case GOT_DISC:
      frog_deal_with_disc(&bi);	/* Temporary */
      break;
    case CHAOS:
      if (bi.numdiscs)
	frog_go_get_the_disc(&bi,0);
      else
	frog_move(&bi);
      break;
    default:
      frog_what_the_fuck(&bi);
      break;
    }
   ...
   Binfo *bi;
{
  ...
  newdir = frog_manuever(bi,bi->dir);
     ...
     Binfo *bi;
{
  ...
  if (!bi->discowned) {
	...
	bi->midstride = FALSE;
	frog_throw_towards(bi,&goal_loc,TRUE);
	return;
      }
      else {
  ...
  if (bi->numvehicles) {
	...
	if (clear_path(&bi->dinfo[0].loc,&bi->vinfo[i].loc)) {
	  ...
	  if (distsqr > BOX_WIDTH*BOX_WIDTH*2) {
	    ...
	    frog_throw_towards(bi,&bi->vinfo[i].loc,TRUE);
	  }
	}
      }
    }
  }
  frog_play_keep_away(bi);
  frog_move2(bi,bi->dir);
}
...
frog_throw_towards(bi,tloc,newthrow)
     Binfo *bi;
     Location *tloc;
     int newthrow;
{
...
frog_go_get_the_disc(bi,discnum)
   Binfo *bi;
   int discnum;
{
  int dx,dy;
  ...
  if (owner == NO_OWNER || get_team(owner) != bi->me.team) {
    ...
    while (i++ < DISC_FRAMES) {
      frog_update_disc(bi,&discfuture,&bi->dinfo[discnum]);
...
*/
frog_update_disc(bi,discloc,discinfo)
     Binfo *bi;
     Coord *discloc;
     Disc_info *discinfo;
{
...
/* frog_wall is our improved version of wall,....
** used by frog_turn, of course
*/
frog_wall(bi, side, x, y)
   Binfo *bi;
   WallSide side;
   int x,y;
{
...
Angle frog_turn(bi,dir)
     Binfo *bi;
     Angle dir;
{
  if (dir > PI/4 && dir < 3*PI/4) {	        /* goin' south */
    if (!frog_wall(bi,SOUTH,bi->me.loc.grid_x,bi->me.loc.grid_y)) {
      return(NO_CHANGE);
    } 
    else if (bi->me.loc.box_y > BOX_HEIGHT - TOO_CLOSE) {
      if (!frog_wall(bi,WEST,bi->me.loc.grid_x,bi->me.loc.grid_y))
	return(PI);
      else if (!frog_wall(bi,EAST,bi->me.loc.grid_x,bi->me.loc.grid_y))
	return(0.0);
      else return(3*PI/2);
    }
    ...
    if (!frog_wall(bi,WEST,bi->me.loc.grid_x,bi->me.loc.grid_y)) {
      ...
      if (!frog_wall(bi,NORTH,bi->me.loc.grid_x,bi->me.loc.grid_y))
	return(3*PI/2);
      else if (!frog_wall(bi,SOUTH,bi->me.loc.grid_x,bi->me.loc.grid_y))
	return(PI/2);
      else return(0.0);
    }
    else return(NO_CHANGE);
    ...
    if (!frog_wall(bi,NORTH,bi->me.loc.grid_x,bi->me.loc.grid_y)) {
      ...
      if (!frog_wall(bi,EAST,bi->me.loc.grid_x,bi->me.loc.grid_y))
	return(0.0);
      else if (!frog_wall(bi,WEST,bi->me.loc.grid_x,bi->me.loc.grid_y))
	return(PI);
      else return(PI/2);
    }
    else return(NO_CHANGE);
    ...
    if (!frog_wall(bi,EAST,bi->me.loc.grid_x,bi->me.loc.grid_y)) {
      ...
      if (!frog_wall(bi,SOUTH,bi->me.loc.grid_x,bi->me.loc.grid_y))
	return(PI/2);
      else if (!frog_wall(bi,NORTH,bi->me.loc.grid_x,bi->me.loc.grid_y))
	return(3*PI/2);
      else return(PI);
    }
    else return(NO_CHANGE);
     ...
     Angle dir;
{
  ...
  switch (corner) {
  case NW:
    if (frog_wall(bi,NORTH,gx,gy)) {
      if (frog_wall(bi,WEST,gx,gy)) 
	doturnstuff(atanint(BOX_HEIGHT - by, bx), PI, -1,
		    atanint(BOX_WIDTH - bx, by), 3*PI/2, 1);
      else {
	if (frog_wall(bi,NORTH,gx - 1,gy))
    ...
    else if (frog_wall(bi,WEST,gx,gy)) {
      if (frog_wall(bi,WEST,gx,gy - 1))
	doturnstuff(atanint(BOX_HEIGHT - by, bx), PI, -1,
    ...
    else if (frog_wall(bi,WEST,gx,gy - 1)) {
      if (frog_wall(bi,NORTH,gx - 1,gy)) 
	doturnstuff(atanint(BOX_WIDTH + bx, by), 3*PI/2, -1,
		    atanint(BOX_HEIGHT + by, bx), PI, 1);
      else 
    ...
    else if (frog_wall(bi,NORTH,gx - 1,gy)) {
      doturnstuff(atanint(BOX_WIDTH + bx, by), 3*PI/2, -1,
		  atanint(bx, by), 3*PI/2, -1);
    }
    break;
  case SW:
    if (frog_wall(bi,SOUTH,gx,gy)) {
      if (frog_wall(bi,WEST,gx,gy))
	doturnstuff(atanint(BOX_WIDTH - bx,BOX_HEIGHT - by), PI/2, -1,
		    atanint(by,bx), PI, 1);
      else {
	if (frog_wall(bi,SOUTH,gx - 1,gy))
	  doturnstuff(atanint(BOX_WIDTH - bx,BOX_HEIGHT - by), PI/2, -1,
    ...
    else if (frog_wall(bi,SOUTH,gx - 1, gy)) {
      doturnstuff(atanint(bx,BOX_HEIGHT - by),PI/2,1,
		  atanint(BOX_WIDTH + bx,BOX_HEIGHT - by), PI/2, 1);
    }
    break;
  case NE:
    ...
    else if (frog_wall(bi,EAST,gx,gy)) {
      if (frog_wall(bi,EAST,gx,gy - 1))
    ...
    else if (frog_wall(bi,EAST,gx,gy - 1)) {
      if (frog_wall(bi,NORTH,gx + 1,gy))
	doturnstuff(atanint(BOX_HEIGHT + by,BOX_WIDTH - bx), 2*PI, -1,
		    atanint(2*BOX_WIDTH - bx,by), 3*PI/2, 1);
      else

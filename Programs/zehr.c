...
typedef struct 
{
  ...
  int mode, frame, next_frame, crashed, last_dir;
  ...
  int destination_dir[40], destination_length, cur_weapon;
} Everything;

extern int zehr_main();
...
zehr_main()
{
  ...
  all.num_blips = all.num_bullets = all.num_vehicles = 0;
  all.mode = EXPLORING;
  all.cur_weapon = 0;
  all.last_dir = -1;
  ...
  ptr = &all.maze[GRID_WIDTH+1][GRID_HEIGHT+1] + 1;
  while((unsigned int) ptr > (unsigned int) all.maze)
    *(ptr--) = (unsigned char) 0;

  ptr = &all.maze_search[GRID_WIDTH+1][GRID_HEIGHT+1] + 1;
  while((unsigned int) ptr > (unsigned int) all.maze_search)
    *(ptr--) = (unsigned char) NULL_DIRECTION;

  while(1){
    frame = frame_number();
    if (all.frame == frame) 
      {
	done();    /* wait until next frame */
	frame = frame_number();
      }
    ...
    switch(all.mode)
      {
      case EXPLORING:
	/* in each case, there should be some processing of data
	   in order to decide what the new timing values should be */
	...
	if (frame >= all.timing.next_vehicle)
	  {
	    zehr_get_vehicles(&all);
	    all.timing.next_vehicle = frame + 8;
	    if (all.num_vehicles) all.mode = COMBAT;
	  }
	...
	if (frame >= all.timing.next_vehicle)
	  {
	    zehr_get_vehicles(&all);
	    if (!(all.num_vehicles)) all.mode = YELLOW;
	    zehr_combat_target(&all);
     ...
     Everything *all;
{
  ...
  switch(all->mode)
    {
    case EXPLORING:
      dir = zehr_explore_dir(all);
      break;
    case NEED_HELP:
    case COMBAT:
      dir = zehr_combat_dir(all);
      break;
    case RESTING:
    case YELLOW:
      dir = zehr_yellow_dir(all);
      break;
    }

  zehr_set_dest(all, dir);

  /* don't come back until generic_move calls, except in combat */
  if (all->mode == COMBAT) all->timing.next_move = all->frame + 10;
  else all->timing.next_move = all->frame + 25;
}


zehr_explore_dir(all)
     Everything *all;
{
  int dir, total, chances[4], x,y, choice, i;

  /* tries to make an intelligent choice of next box to go to
     while in explore mode.  takes into account whether the box has
     been visited before, how close it is to the edge of the maze,
     and other factors */
  ...
  if (!(all->maze[x][y] & W_WALL))
    {
      chances[WEST] = 1;
      if (!(all->maze[x-1][y] & VISITED)) chances[WEST] += 100;
      if (all->last_dir != EAST) chances[WEST] += 10;
    }
  if (!(all->maze[x][y] & E_WALL))
    {
      chances[EAST] = 1;
      if (!(all->maze[x+1][y] & VISITED)) chances[EAST] += 100;
      if (all->last_dir != WEST) chances[EAST] += 10;
    }
  if (!(all->maze[x][y] & N_WALL))
    {
      chances[NORTH] = 1;
      if (!(all->maze[x][y-1] & VISITED)) chances[NORTH] += 100;
      if (all->last_dir != SOUTH) chances[NORTH] += 10;
    }
  if (!(all->maze[x][y] & S_WALL))
    {
      chances[SOUTH] = 1;
      if (!(all->maze[x][y+1] & VISITED)) chances[SOUTH] += 100;
      if (all->last_dir != NORTH) chances[SOUTH] += 10;
    }

  for(i=0;i<all->num_blips;i++)
    {
      if (all->blip[i].x > x) { if (chances[EAST]) chances[EAST] += 200; }
      else if (chances[WEST]) chances[WEST] +=200;
      if (all->blip[i].y > y) { if (chances[SOUTH]) chances[SOUTH] +=200; }
      else if (chances[NORTH]) chances[NORTH] +=200;
    }

  /* pick a random direction based on weighting */
  total = chances[NORTH] + chances[SOUTH] + chances[WEST] + chances[EAST];
  dir = 0;
  choice = rand() % total;
  while(choice >= chances[dir])
      choice -= chances[dir++];
  return(dir);
}
...
zehr_combat_dir(all)
     Everything *all;
{
  int dir, total, chances[4], x,y, choice,i, x1,y1, dx,dy;
  int dist, inc;

  /* tries to make an intelligent choice of next box to go to
     based on where our "target" is and where the other vehicles
     are, trying to maintain a constant distance from the enemy
     but not get trapped */

  x = all->loc.grid_x;
  y = all->loc.grid_y;

  chances[WEST] = chances[EAST] = chances[NORTH] = chances[SOUTH] = 0;
  if (!(all->maze[x][y] & W_WALL)) chances[WEST] = 1;
  if (!(all->maze[x][y] & E_WALL)) chances[EAST] = 1;
  if (!(all->maze[x][y] & N_WALL)) chances[NORTH] = 1;
  if (!(all->maze[x][y] & S_WALL)) chances[SOUTH] = 1;
  
  x1 = all->me.x;
  y1 = all->me.y;

  /* look at each of the visible or on-radar vehicles, and adjust
     the chance of going in a certain direction based on where that
     vehicle is and how fast it's moving, etc. */

  /* what it does now:  it tries to maintain a distance of one box 
     from the target vehicle.  it attempts to move away from non-target
     vehicles */
  ...
  for(i=0;i<all->num_vehicles;i++)
      ...
      if (all->vehicle[i].id == all->target_id)
	{
	  if ((dx > BOX_WIDTH) || ((dx < 0) && (dx > -BOX_WIDTH)))
	    {if (chances[EAST]) chances[EAST] += 50;}
	  else if (chances[WEST]) chances[WEST] += 50;

	  if ((dy > BOX_HEIGHT) || ((dy < 0) && (dy > -BOX_HEIGHT)))
	    {if (chances[SOUTH]) chances[SOUTH] += 50;}
	  else if (chances[NORTH]) chances[NORTH] += 50;

	  dist = (int) sqrt((double) (dx*dx + dy*dy));
	  if (dist > BOX_WIDTH) all->desired_speed = 5.0;
	  else if (dist > BOX_WIDTH/2) all->desired_speed = 6.0;
	  else if (dist > BOX_WIDTH/4) all->desired_speed = 7.3;
	  else all->desired_speed = 9.0;
	}
      else
	{
	  dist = dx*dx + dy*dy;
	  if (dist < 1000) inc = 20;
	  else if (dist < 10000) inc = 10;
	  else inc = 4;
	  if (x > x1) { if (chances[WEST]) chances[WEST] += inc; }
	  else if (chances[EAST]) chances[EAST] += inc;
	  if (y>y1) { if (chances[NORTH]) chances[SOUTH] += inc; }
	  else if (chances[SOUTH]) chances[SOUTH] += inc;
	}
    }

  for(i=0;i<all->num_blips;i++)
    {
      if (all->blip[i].x > x1) { if (chances[WEST]) chances[WEST]++; }
      else if (chances[EAST]) chances[EAST]++;
      if (all->blip[i].y > y1) { if (chances[NORTH]) chances[SOUTH]++; }
      else if (chances[SOUTH]) chances[SOUTH]++;
    }
     ...
     Everything *all;
{
  ...
  for(i=0;i<all->num_blips;i++)
	...
	dist = all->blip[i].x - all->loc.grid_x;
      else dist = all->loc.grid_x - all->blip[i].x;
      if (all->blip[i].y > all->loc.grid_y)
	dist += all->blip[i].y - all->loc.grid_y;
      else dist += all->loc.grid_y = all->blip[i].y;
      if (dist < max_dist)
	{
	  max_dist = dist;
	  target = i;
  ...
  zehr_plot_course(all, all->loc.grid_x, all->loc.grid_y,
     ...
     Everything *all;
{
  if (all->destination_length == 0) 
    {
      return(zehr_explore_dir(all));
    }
  else return(all->destination_dir[--all->destination_length]);
}
     ...
     Everything *all;
{
  ...
  for(i=x-1;i<=x+1;i++)
    for(j=y-1;j<=y+1;j++)
      {
	if (all->maze[i][j] & SCANNED) continue;
	if (wall(EAST,i,j))
	  {
...
zehr_generic_move(all)
     Everything *all;
{
  /* this is what get's called every frame (I hope)
     it checks for collisions, and other "special cases"
     otherwise, it heads for the current all->dest
     the type of motion (straight, etc) depends on the current mode
     it assumes that there is a straight path to all->dest.
     all->dest will be kept up to date by a movement routine which is
     called periodically, and that will head for all->final_dest.
     this routine will schedule the next call to the short movement routine
  ...
  if (dx == 0)
    if (dy < 0) angle = 1.5*PI;
    else angle = 0.5*PI;
  else angle = (float) atan2((double)dy,(double)dx);

  switch(all->mode)
    {
    case EXPLORING:
      zehr_try_to_go(all, angle, 8.0);
      break;
    case NEED_HELP:
    case COMBAT:
      zehr_try_to_go(all, angle + (rand() % 40)/100 - 0.20, 
		     all->desired_speed);
      break;
    case RESTING:
    case YELLOW:
      zehr_try_to_go(all, angle, 6.0);
      break;
    }
}


zehr_try_to_go(all, angl, speed)
     Everything *all;
     float angl,speed;
{
  ...
  if (theta > 0.25*PI) speed = 3.0;
  if (speed != all->last_speed) set_rel_drive(speed);
...
zehr_mode(all)
     Everything *all;
{
  /* this should be one of the "smarter" parts of the program, but
     right now it's pretty dumb */
...
zehr_plot_course(all, x,y,x1,y1)
     Everything *all;
     int x,y,x1,y1;
{
  /* plots a course from x,y to x1,y1, trying to find close to
     the shortest path, while staying within scanned territory */
  ...
  if (min_distance < 4) max_distance = min_distance << 1;
  else max_distance = 8;
  ...
  if (shortest_point == -1)
    for(i=0;i<new_end;i++)
      {
	if (x1 < data_x[i]) dist = data_x[i] - x1;
	else dist = x1 - data_x[i];

	if (y1 < data_y[i]) dist += data_y[i] - y1;
	else dist += y1 - data_y[i];
     ...
     Everything *all;
{
  ...
  target = 0;
  for(i=0;i<all->num_vehicles;i++)
    if (all->vehicle[i].id == all->target_id) target = i;
  ...
  if (all->cur_weapon > 2) all->cur_weapon = 0;
  else ++all->cur_weapon;
  ...
  if (zehr_clear_path(all, &all->loc, &tloc))
    {
      aim_all_turrets(dx, dy);
      if (dx*dx + dy*dy < RANGE*RANGE) fire_all_weapons();
    } 
}
...
*/
zehr_clear_path(all, start,finish)
     Everything *all;
  ...
  */
  if(fgrid_x > tgrid_x)
	...
	if(lattice_dy * dx < dy * lattice_dx) {
	  if(all->maze[tgrid_x][tgrid_y] & S_WALL) return(0);
	  tgrid_y++;
	  lattice_dy += BOX_HEIGHT;
	}
	else {
	  if(all->maze[tgrid_x][tgrid_y] & E_WALL) return(0);
	  tgrid_x++;
      ...
      while(tgrid_x != fgrid_x || tgrid_y != fgrid_y) {
	if(lattice_dy * dx > dy * lattice_dx) {
	  if(all->maze[tgrid_x][tgrid_y] & N_WALL) return(0);
	  tgrid_y--;
	  lattice_dy -= BOX_HEIGHT;
	}
	else {
	...
	if(lattice_dy * dx >= dy * lattice_dx) {
	  ...
	  if(all->maze[tgrid_x][tgrid_y] & W_WALL) return(0);
	  tgrid_x--;
	  lattice_dx -= BOX_WIDTH;
	}
      }

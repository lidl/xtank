...
typedef struct
{
	void *next;					/* pointer to next in a linked list */
	GraphicType type;
	union
	{
		LineGraphic line;
...
thud_main()
{
	...
	bzero(allp, sizeof(*allp));	/* clear all fields to zero */
		...
		for (i = num_weapons() - 1; i >= 0; --i)
		  {
		    ...
		    get_weapon(i, &wi);
		    if ((wi.range > allp->max_range[wi.mount]) && (cv->weapon[i].status&WS_on))
		      {
			allp->max_range[wi.mount] = wi.range;
			memcpy(&allp->winfo[wi.mount], &wi, sizeof(wi));
		      }
		  }

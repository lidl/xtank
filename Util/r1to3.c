...
** r1to3.c
**
** This program reads in an X10 formatted bitmap files and
** writes out one flipped and rotated 90 degrees
     ...
     char *argv[];
{
  ...
  for(j = 0 ; j < num_shorts ; j++)
    fscanf(bitmap_file,"%*2s%4x%*s",&rotation[1][j]);
  fclose(bitmap_file);

  /* Flip rotation 1 on vertical axis, and rotate by 90 degrees */
  ...
  fprintf(bitmap_file,"#define %s3_width %d\n",filename,width);
  fprintf(bitmap_file,"#define %s3_height %d\n",filename,height);
  fprintf(bitmap_file,"static short %s3_bits[] = {",filename);
  for(j = 0 ; j < num_shorts ; j++) {
    if(j%8 == 0) fprintf(bitmap_file,"\n    ");
    if(j != num_shorts - 1) fprintf(bitmap_file,"0x%04x, ",rotation[3][j]);
